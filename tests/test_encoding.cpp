...
TEST(TestEncoding, TestHexEncoding) {
    ...
    EXPECT_EQ((ErrorCode)bv_err_de_1.error().value().error_code(),
              (ErrorCode)EncodingErrorCodeEnum::HexBadDecodingSource);
    auto bv_err_de_2 = he.decode("A123C");
    EXPECT_EQ((ErrorCode)bv_err_de_2.error().value().error_code(),
              (ErrorCode)EncodingErrorCodeEnum::HexBadDecodingSource);
}
...
TEST(TestEncoding, TestBase64Encoding) {
    ...
    EXPECT_EQ((ErrorCode)err_res.error().value().error_code(),
              (ErrorCode)EncodingErrorCodeEnum::Base64BadDecodingSource);
}
